...
export async function unlock(masterPassword) {
  ...
  // LIVE MODE (wired after the crypto/backend decision):
  //   const bw = await import("./bitwarden.js");
  //   const items = await bw.unlockAndSync(masterPassword);
  //   await session.set(SESSION_VAULT, items); ...
  return { ok: false, error: "Live sync not yet wired — see ADR 0001." };
}
...
export async function addItem(partial) {
  ...
  items.unshift(item);
  await session.set(SESSION_VAULT, items);
  // LIVE MODE: also POST /api/ciphers and reconcile by revisionDate.
  return item;
}
